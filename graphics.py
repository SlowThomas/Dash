...
def update(rate=None):
    ...
    if rate:
        ...
        if pauseLength > 0:
            ...
            real_rate = 1/(now-_update_lasttime)
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        ## modified by Thomas ####################################################
        self.mouseHoverX = None
        self.mouseHoverY = None
        self.bind("<Motion>", self._onHover)
        self.bind("<Leave>", self._onMouseLeave)
        self.mouseDragX = None
        self.mouseDragY = None
        self.bind("<B1-Motion>", self._onDrag)
        self.bind("<ButtonRelease-1>", self._onDragRelease)
        self.all_keys = []
        self.bind("<KeyPress>", self._onKeyPressed)
    ...
    ## modified by Thomas ####################################################
    def _onDrag(self, e):
        self.mouseHoverX = e.x
        self.mouseHoverY = e.y
        self.mouseDragX = e.x
        self.mouseDragY = e.y

    def _onDragRelease(self, e):
        self.mouseDragX = None
        self.mouseDragY = None
    ...
    def checkMouseHover(self):
        self.update()
        if self.mouseHoverX != None and self.mouseHoverY != None:
            x, y = self.toWorld(self.mouseHoverX, self.mouseHoverY)
        else:
            return None
        return Point(x, y)

    def checkMouseDrag(self):
        self.update()
        if self.mouseDragX != None and self.mouseDragY != None:
            x, y = self.toWorld(self.mouseDragX, self.mouseDragY)
        else:
            return None
        return Point(x, y)

    def checkAllKeys(self):
        self.update()
    ...
    def _onKeyPressed(self, e):
        if not (e.keysym in self.all_keys):
            self.all_keys.append(e.keysym)

    def _onKeyRelease(self, e):
        if e.keysym in self.all_keys:
            self.all_keys.pop(self.all_keys.index(e.keysym))
    ## modified by Thomas ####################################################
